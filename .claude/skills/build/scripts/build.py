...
FEATURE_SIDEBAR = [
    ...
    ("features/native.html", "原生波特人", "第一層：直接用"),
    ("features/hatcher.html", "孵化器（Hatcher）", "第二層：自己建 Bot"),
    ("features/bots-detail.html", "波特小隊與記憶", "第三層：專業 Bot"),
    ("features/platform.html", "平台能力", "平台"),
]
...
def get_sidebar_html(current_path: str) -> str:
    ...
    items = []
    last_group = None
    for site_path, label, group in FEATURE_SIDEBAR:
        # 分組標題
...
def is_feature_page(site_path: str) -> bool:
    """判斷是否為功能頁面（需要側邊欄）"""
    return site_path.startswith("features/")
...
def render_page(meta: dict, body_html: str, site_path: str) -> str:
    ...
    has_sidebar = is_feature_page(site_path)
